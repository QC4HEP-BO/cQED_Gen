...
def test_three_qubit_capacitive_star_expected_current_behavior() -> None:
    # Cycle traversal is deterministic from the selected root. The remaining
    # two-node tail Cc_23 -> T2 is now directionally compressed as TC(dir=-1).
    compressed = graphlize(build_three_qubit_capacitive_star())
    assert compressed_type_names(compressed) == ["C_COUPLER", "TC", "TCT"]


def test_directional_two_node_order_is_preserved() -> None:
    # F-C-R -> F-CR: same RC SubgType, reverse direction marker.
    t = CQEDTopology("f_c_r")
    f = t.add_node(SubgType.FEEDLINE, {}, label="F")
    c = t.add_node(SubgType.C_COUPLER, _cc_attrs(1e-15), label="C")
    r = t.add_node(SubgType.RESONATOR, _r_attrs(1.0), label="R")
    t.add_edge(f, c); t.add_edge(c, r)
    compressed = graphlize(t)
    rc = next(n for n in compressed._nodes if n.subg_type == SubgType.RC)
    assert rc.label == "RC(C+R)"
    assert rc.attrs["dir"] == -1.0

    # F-R-C -> F-RC: same RC SubgType, canonical direction marker.
    t = CQEDTopology("f_r_c")
    f = t.add_node(SubgType.FEEDLINE, {}, label="F")
    r = t.add_node(SubgType.RESONATOR, _r_attrs(1.0), label="R")
    c = t.add_node(SubgType.C_COUPLER, _cc_attrs(1e-15), label="C")
    t.add_edge(f, r); t.add_edge(r, c)
    compressed = graphlize(t)
    rc = next(n for n in compressed._nodes if n.subg_type == SubgType.RC)
    assert rc.label == "RC(R+C)"
    assert rc.attrs["dir"] == 1.0
