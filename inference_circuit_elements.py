...
def _unique_attr_names_from_graph(g):
    ...
    In the dir-classifier/topological-loss model, ``dir`` is not a continuous
    physical parameter anymore: it is predicted by the topological decoder.
    Therefore it must be excluded from the flattened attribute matrix before
    calling the parameter scaler inverse_transform().

    Duplicate non-dir attributes are made unique by appending a node-index suffix
    when the same base attribute name was already emitted by a different node.
    """
    names = []
...
def _attr_node_indices_from_graph(g):
    """Return the node index for each flattened non-dir attribute column.

    The order exactly matches _unique_attr_names_from_graph(), sample.y and the
    scaler columns.  This lets us aggregate per-parameter metrics back onto the
    graph nodes for the NetworkX diagnostic plot.
    """
    node_indices = []
    for node_i, st_int in enumerate(g.node_types):
        for attr_name in SUBG_DEFS[SubgType(st_int)].attrs:
...
def _scaled_attr_row_from_pred_graph(g_pred):
    """Flatten predicted non-dir attrs by node and SUBG_DEFS order."""
    row = []
    for node_i, st_int in enumerate(g_pred.node_types):
        ...
        for attr_name in attrs_ordered:
            if attr_name == "dir":
                continue
...
def _dir_value_from_attrs(attrs: dict | None) -> float:
    """Return macro-node direction as float, accepting both dir and direction keys."""
    if not attrs:
        return float("nan")
...
def dir_match(g_true, g_pred) -> dict:
    """Compare predicted macro-node dir against the true macro graph.

    Direction is evaluated only on true macro nodes that carry a dir/direction
    attribute.  A predicted value is considered valid only when the node index
    exists, the predicted node type matches the true node type, and the decoded
    graph exposes dir/direction for that node.
    """
    ...
    true_attrs = getattr(g_true, "attrs", []) or []
    pred_attrs = getattr(g_pred, "attrs", []) or []
    true_types = getattr(g_true, "node_types", []) or []
    pred_types = getattr(g_pred, "node_types", []) or []

    for i, attrs in enumerate(true_attrs):
        true_dir = _dir_value_from_attrs(attrs)
        if not np.isfinite(true_dir):
            continue

        true_dirs.append(true_dir)
        pred_dir = float("nan")
        if i < len(pred_types) and i < len(pred_attrs) and i < len(true_types):
            if int(pred_types[i]) == int(true_types[i]):
                pred_dir = _dir_value_from_attrs(pred_attrs[i])

        pred_dirs.append(pred_dir)
        if np.isfinite(pred_dir):
            diff = float(pred_dir - true_dir)
            diffs.append(diff)
            if (pred_dir >= 0.0) == (true_dir >= 0.0):
                correct += 1

    return {
        "dir_n_true": len(true_dirs),
        "dir_n_valid": len(diffs),
        "dir_n_correct": correct,
        "dir_true": true_dirs,
        "dir_pred": pred_dirs,
        "dir_diff": diffs,
    }
    ...
    for ds_name, samples in by_ds.items():
        ...
        for i in range(0, len(samples), batch_size):
            ...
            for s, g_pred in zip(batch, graphs_pred):
                ...
                tm.update(dir_match(g_true, g_pred))
...
def compute_topo_metrics(topo_results: list[dict]) -> dict:
    ...
    dir_n_true = sum(int(r.get("dir_n_true", 0)) for r in topo_results)
    dir_n_valid = sum(int(r.get("dir_n_valid", 0)) for r in topo_results)
    dir_n_correct = sum(int(r.get("dir_n_correct", 0)) for r in topo_results)
    dir_diffs = [float(d) for r in topo_results for d in r.get("dir_diff", []) if np.isfinite(d)]
    dir_acc = dir_n_correct / dir_n_valid if dir_n_valid > 0 else float("nan")
    dir_coverage = dir_n_valid / dir_n_true if dir_n_true > 0 else float("nan")
    dir_mae = float(np.mean(np.abs(dir_diffs))) if dir_diffs else float("nan")
    dir_mean_diff = float(np.mean(dir_diffs)) if dir_diffs else float("nan")
    ...
    return {
        ...
        "dir_mae": dir_mae,
        "dir_mean_diff": dir_mean_diff,
        "dir_n_true": dir_n_true,
        "dir_n_valid": dir_n_valid,
        "dir_n_correct": dir_n_correct,
        "n_samples": n,
...
def print_report(branch_name: str, results: dict, decode_used_ds_ids: bool, ds_map: dict[str, int]) -> None:
    ...
    for ds_name in sorted(results):
        ...
        print(f"    Dir Accuracy       : {_fmt(tm_metrics['dir_accuracy'])}  "
              f"({tm_metrics['dir_n_correct']}/{tm_metrics['dir_n_valid']} valide; "
              f"coverage={_fmt(tm_metrics['dir_coverage'])})")
        print(f"    Dir MAE / mean Δ   : {_fmt(tm_metrics['dir_mae'])} / {_fmt(tm_metrics['dir_mean_diff'])}")
...
def main() -> None:
    ...
    if args.only_latent:
        ...
        train_data, val_data, test_data, fresh_scalers = load_all_datasets_vae(
        ...
        # Merge checkpoint scalers with scalers freshly fitted by the loader.
        # This makes inference robust to datasets/topology names that were not
        # present when the checkpoint was saved.  Existing checkpoint scalers are
        # kept as the source of truth for datasets seen during training; only
        # missing keys fall back to fresh_scalers.
        missing_scalers = [ds_name for ds_name in fresh_scalers if ds_name not in scalers]
        if missing_scalers:
            print("  ATTENZIONE: scaler mancanti nel checkpoint; uso scaler freschi per:")
            for ds_name in missing_scalers:
                print(f"    - {ds_name}")

        merged_scalers = dict(fresh_scalers)
        merged_scalers.update(scalers)
        scalers = merged_scalers
    ...
    if not args.only_spec and not args.only_latent:
        ...
        t0 = time.time()
        results_circ, used_ds_ids_c, ds_map_c = run_circuit_encoder(model, data, scalers, device, batch_size=args.batch_size, stochastic=args.stochastic, use_permutation_eval=not args.no_permutation_eval)
        ...
        if not args.no_plot_r2_graph:
            plot_param_r2_graph(results_circ, str(out_dir / "inference_circuit_enc_r2_graph.png"), "circuit-enc", args.ckpt)

    if not args.only_circuit and not args.only_latent:
        print("\n[Ramo B] Spec encoder  obs -> z^s -> decoder...")
        t0 = time.time()
        results_spec, used_ds_ids_s, ds_map_s = run_spec_encoder(model, data, scalers, device, batch_size=args.batch_size, stochastic=args.stochastic, use_permutation_eval=not args.no_permutation_eval)
        print(f"  completato in {time.time() - t0:.1f}s")
        ...
        if not args.no_plot_r2_graph:
            plot_param_r2_graph(results_spec, str(out_dir / "inference_spec_enc_r2_graph.png"), "spec-enc", args.ckpt)
